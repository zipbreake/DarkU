 ...
 *
 * $Id: packet.c,v 1.1.1.1 2006/05/27 13:54:18 zipbreake Exp $
 */
#include "config.h"
...
/* 
  Compresión de enlaces servidor <=> servidor extraída del ircu del IRC-Hispano http://www.irc-hispano.org */
int server_dopacket(struct Client* cptr, char* buffer, int length)
{
  char*		  src;
  char*       endp;
  char*       client_buffer;

  int encr = !!(cli_negociacion(cptr) & NEGOCIACION_RC4_IN);
  ...
  if (encr)
	  rc4_process_stream((void *)cptr->estado_rc4_in, buffer, length);
    ...
    *endp = *src++;
    ...
    if (IsEol(*endp)) {
      ...
      *endp = '\0';
	  ...
	  /* Se empieza a recibir encriptado aquí */
	  if (!encr && (cli_negociacion(cptr) & NEGOCIACION_RC4_IN)) {
		  while ((--length >= 0) && ((*src == '\r') || (*src == '\n')))
			  src++;
		  rc4_process_stream((void *)cptr->estado_rc4_in, src, ++length);
		  encr = 1;
	  }
    }
    ...
    *endp = *src++;
    ...
    if (IsEol(*endp))
    {
      ...
      *endp = '\0';
      ...
      {
        cli_count(cptr) = 0;
        return server_dopacket(cptr, (char *)src, length);
      }
      endp = client_buffer;
    }
    else if (endp < client_buffer + BUFSIZE)
