 ...
 *
 * $Id: m_oper.c,v 1.5 2009/12/03 09:46:13 zipbreake Exp $
 */
...
CMD_FUNC(m_oper)
{
  ...
  if (EmptyString(name) || EmptyString(password)) {
	if (IsAdmin(sptr) || IsBot(sptr)) {
	    unsigned int old_mode = (cli_flags(sptr) & ALL_UMODES);
	    unsigned int old_rhmode = (cli_rhflags(sptr) & RHFLAGS);
	    ...
	    send_umode_out(cptr, sptr, old_mode, old_rhmode, HasPriv(sptr, PRIV_PROPAGATE));
	    send_reply(sptr, RPL_YOUREOPER);
	    sendto_opmask_butone(0, SNO_OLDSNO, "%s (%s@%s) es ahora un operador del IRC (ADMIN)",
  ...
  if (oper_password_match(password, aconf->passwd)) {
    ...
    sendto_opmask_butone(0, SNO_OLDSNO, "%s (%s@%s) es ahora un operador del IRC (%c)",
			 parv[0], cli_user(sptr)->username, cli_sockhost(sptr),
			 IsOper(sptr) ? 'O' : 'o');
