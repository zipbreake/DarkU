 ...
 *
 * $Id: channel.c,v 1.19 2010/04/29 07:47:20 zipbreake Exp $
 */
#include "config.h"
...
*/  if (chptr->mode.limit) {
  ...
  if (chptr->rhmode.mode & RHMODE_NOCOLOR)
	  *mbuf++ = 'c';
  if (chptr->rhmode.mode & RHMODE_NOCOLORNOT)
	  *mbuf++ = 'C';
  if (chptr->rhmode.mode & RHMODE_NONOTICES)
	  *mbuf++ = 'N';
  if (chptr->rhmode.mode & RHMODE_RGSTRONLY)
	  *mbuf++ = 'R';
  if (chptr->rhmode.mode & RHMODE_SECURE)
	  *mbuf++ = 'Z';
...
void send_channel_modes(struct Client *cptr, struct Channel *chptr)
{
  ...
  *modebuf = *parabuf = '\0';
  ...
  for (first = 1; full; first = 0)      /* Loop for multiple messages */
  {
     ...
     *
     * Run 8 times over all members, to group the members with the
     * same mode together
     */
	...
	if (msgq_bufleft(mb) < NUMNICKLEN + 7)
          /* The 7 is a possible ",:ovhqa" */
        {
...
/*  if (IsLocalChannel(chptr->chname) && HasPriv(sptr, PRIV_WALK_LCHAN) &&
  ...
  if (IsAnOper(sptr) && !BadPtr(key) && compall("GOD",key) == 0)
    overrideJoin = MAGIC_OPER_OVERRIDE;

  if (es_representante(sptr) && !BadPtr(key) && compall("OPER",key) == 0)
    overrideJoin = MAGIC_OPER_OVERRIDE;
  ...
  if ((chptr->rhmode.mode & RHMODE_SECURE) && !(IsRegnick(sptr) || IsSecured(sptr) || IsWebchatUser(sptr)))
  	return overrideJoin + ERR_NEEDSECURENICK;
...
/*	  RHMODE_HALFOP,	'h',*/
  ...
  for (i = 0; i < mbuf->mb_count; i++) {
    ...
    if (MB_TYPE(mbuf, i) & (MODE_CHANOP | MODE_VOICE | CHFL_HALFOP)) {
...
modebuf_rhmode(struct ModeBuf *mbuf, unsigned int mode)
{
  ...
  mode &= (MODE_ADD | MODE_DEL | RHMODE_BADWORDS | RHMODE_NOCOLOR | RHMODE_NOCOLORNOT | RHMODE_RGSTRONLY | RHMODE_SECURE |
	   RHMODE_SUSPEND | RHMODE_REGMOD | RHMODE_ONLYIRCOP | RHMODE_NONOTICES |
...
*/
	...
	member->status |= (state->cli_change[i].flag &
			   (MODE_CHANOP | MODE_VOICE | CHFL_HALFOP));
...
*/
	...
	break;
      case 'O':
	if (!IsAnOper(sptr) && !IsServer(sptr) && !IsBot(sptr))
	    break;
