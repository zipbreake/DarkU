 ...
 *
 * $Id: m_lusers.c,v 1.1.1.1 2006/05/27 13:53:54 zipbreake Exp $
 */
...
CMD_FUNC(m_lusers)
{
  ...
  snoticeto_one(sptr, "Record de conexiones: %d (%d clientes)",
...
CMD_FUNC(m_users)
{
	if (parc > 2)
		if (hunt_server_cmd(sptr, CMD_USERS, cptr, 0, "%s :%C", 2, parc, parv) !=
			HUNTED_ISME)
		  return 0;
	send_reply(sptr, RPL_LOCAL_USERS, UserStats.local_clients, max_client_count, date(max_client_count_TS));
	send_reply(sptr, RPL_GLOBAL_USERS, UserStats.clients, max_global_count, date(max_global_count_TS));
}
