 ...
 *
 * $Id: m_invite.c,v 1.4 2009/02/08 14:47:17 zipbreake Exp $
 */
...
CMD_FUNC(m_invite)
{
  ...
  if (!find_channel_member(sptr, chptr) && !IsBot(sptr)) {
  ...
  if (!is_chan_op(sptr, chptr) && !is_chan_owner(sptr, chptr) && !is_chan_halfop(sptr, chptr) && !IsBot(sptr)) {
...
CMD_FUNC(ms_invite)
{
  ...
  if (find_channel_member(acptr, chptr) && !IsBot(sptr)) {
    send_reply(sptr, ERR_USERONCHANNEL, cli_name(acptr), chptr->chname);
    return 0;
  }
  add_invite(acptr, chptr);
