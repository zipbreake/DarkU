 ...
 *
 * $Id: m_pong.c,v 1.2 2009/05/10 12:00:46 zipbreake Exp $
 */
...
CMD_FUNC(mr_pong)
{
  ...
  if (0 != cli_cookie(sptr) && COOKIE_VERIFIED != cli_cookie(sptr)) {
    if (parc > 1 && cli_cookie(sptr) == atol(parv[parc - 1])) {
      ...
      if (cli_user(sptr) && *(cli_user(sptr))->realhost && (cli_name(sptr))[0])
        /*
         * NICK and USER OK
         */
        return register_user(cptr, sptr, cli_name(sptr), cli_user(sptr)->username, cli_user(sptr)->realhost);
    }
    else  
