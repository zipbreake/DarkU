 ...
 *
 * $Id: m_join.c,v 1.13 2010/06/03 08:21:49 zipbreake Exp $
 */
...
CMD_FUNC(m_join)
{
  ...
  for (name = ircd_strtok(&p, chanlist, ","); name;
    ...
    /* Puede darse el caso de que sea un canal linkado con otro en un IRCd que lo soporte */
    if (feature_bool(FEAT_LINK_CHANNELS)) {
        if ((reg2 = db2_buscar_subclave(BDD_CHANNELS, name, BDD_CHANNELS_LINK)) && (reg2->valor)) {
             strcpy(name, reg2->valor);
	}
    }
	...
	continue;
      ...
      /* No dejamos crear canales sino procede */
      if ((feature_bool(FEAT_SECURE_CHANNELS)) && !(IsAdmin(sptr) || IsBot(sptr))) {
	send_reply(sptr, ERR_NOTCREATECHANNEL, name);
	continue;
      }
	...
	!HasPriv(sptr, PRIV_CHAN_LIMIT) && !(cli_flags(sptr) & FLAGS_CHSERV)) {
      send_reply(sptr, ERR_TOOMANYCHANNELS, chptr ? chptr->chname : name);
      break; /* no point processing the other channels */
    }

	/* Canales protegidos por tabla j si no es bot */
        if (!IsBot(sptr)) {
	    if ((reg = db_buscar_registro(BDD_JOINS, name)) && (reg->valor) &&
		((req_flags = atol(reg->valor)) > 0L)) {
		if (check_dbaccess(sptr, req_flags) != 2) {
			char motivo[1024];
			sprintf(motivo, "No tienes privilegios (0x%x)", req_flags);
			send_reply(sptr, ERR_CANTJOIN, ((chptr != NULL) ? chptr->chname : name), motivo);
			continue;
		}
	    }
        }
	...
	if (chptr) {
		if ((chptr->rhmode.mode & RHMODE_ONLYIRCOP) && !IsAnOper(sptr)) {
			send_reply(sptr, ERR_CANTJOIN, chptr->chname, "Solo los IRCops pueden entrar en el canal (+O)");
			continue;
		}

      if (check_target_limit(sptr, chptr, chptr->chname, 0) && !(cli_flags(sptr) & FLAGS_CHSERV))
