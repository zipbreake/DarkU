 ...
 *
 * $Id: m_who.c,v 1.3 2009/11/19 22:15:36 zipbreake Exp $
 */
        ...
        nick = ircd_strtok(&p, 0, ","))
    {
      if (IsChannelName(nick) && (chptr = FindChannel(nick)))
      {
		...
		!(IsAnOper(acptr) && /*(*/HasPriv(acptr, PRIV_DISPLAY)/* ||*/
				      /*HasPriv(sptr, PRIV_SEE_OPERS))*/) && !es_representante(acptr))
              continue;
      ...
      else
      {
        if ((acptr = FindUser(nick)) &&
            ((!(bitsel & WHOSELECT_OPER)) || es_representante(acptr) || 
	     (IsAnOper(acptr) && /*(*/HasPriv(acptr, PRIV_DISPLAY)/* ||*/
	  ...
	  if ((bitsel & WHOSELECT_OPER) &&
	    ...
	    continue;
          if ((mask) &&
              ...
              && ((!(matchsel & WHO_FIELD_HOS))
              || matchexec(cli_user(acptr)->virtualhost, mymask, minlen))
              && ((!(matchsel & WHO_FIELD_HOS))
	      || !IsHiddenHost(acptr)
	      || !IsViewip(sptr)
