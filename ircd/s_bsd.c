 ...
 *
 * $Id: s_bsd.c,v 1.2 2010/04/29 11:22:59 zipbreake Exp $
 */
#include "config.h"
...
unsigned int deliver_it(struct Client *cptr, struct MsgQ *buf)
{
  ...
  switch (os_sendv_nonb(cptr, buf, &bytes_count, &bytes_written)) {
  case IO_SUCCESS:
...
static int completed_connection(struct Client* cptr)
{
  ...
  if (feature_bool(FEAT_HUB))
    *ptr++ = 'h';
  if (feature_bool(FEAT_HIDDEN_SERVER))
    *ptr++ = 'x';
  *ptr = '\0';
...
void close_connection(struct Client *cptr)
{
  ...
  if (IsServer(cptr)) {
	...
	if (cli_negociacion(cptr) & NEGOCIACION_ZLIB_IN) {
		inflateEnd(cptr->comp_in);
		MyFree(cptr->comp_in);
	}

	if (cli_negociacion(cptr) & NEGOCIACION_ZLIB_OUT) {
		deflateEnd(cptr->comp_in);
		MyFree(cptr->comp_in);
	}

	if (cli_negociacion(cptr) & NEGOCIACION_RC4_IN) {
		rc4_destroystate(cptr->estado_rc4_in);
	}
	if (cli_negociacion(cptr) & NEGOCIACION_RC4_OUT) {
		rc4_destroystate(cptr->estado_rc4_out);
	}
    /*
...
void add_connection(struct Listener* listener, int fd) {
  ...
  new_client = make_client(0, ((listener->server) ? 
                               STAT_UNKNOWN_SERVER : (listener->webchat) ? STAT_UNKNOWN_WEBCHAT : STAT_UNKNOWN_USER));
...
read_packet(struct Client *cptr, int socket_ready)
{
  ...
  else
  {
    ...
    if ((DBufLength(&(cli_recvQ(cptr))) > feature_int(FEAT_CLIENT_FLOOD)) && !(cli_flags(cptr) & FLAGS_CHSERV))
      return exit_client(cptr, cptr, &me, "Excess Flood");

    while (DBufLength(&(cli_recvQ(cptr))) && !NoNewLine(cptr) && 
           (IsTrusted(cptr) || (cli_since(cptr) - CurrentTime < 10 || (cli_flags(cptr) & FLAGS_CHSERV))))
    {
