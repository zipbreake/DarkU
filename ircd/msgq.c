 ...
 *
 * $Id: msgq.c,v 1.1.1.1 2006/05/27 13:54:14 zipbreake Exp $
 */
#include "config.h"
...
void
msgq_add(struct Client *cptr, struct MsgQ *mq, struct MsgBuf *mb, int prio)
{
...
/* RyDeN - 30 Marzo 2004
   ...
   Procesa un buffer que está listo para enviar con las opciones que tenga el
   destinatario activadas.
*/
void msgq_process(struct Client *acptr, struct iovec *iov, int ioc)
{
    int i;
    ...
    for (i = 0; i < ioc; i++)
    {
        ...
        /* RC4 */
        if (cli_negociacion(acptr) & NEGOCIACION_RC4_OUT)
        {
                rc4_process_stream(acptr->estado_rc4_out, iov[i].iov_base, iov[i].iov_len);
        }
