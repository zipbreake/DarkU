 ...
 *
 * $Id: ircd.c,v 1.1.1.1 2006/05/27 13:53:33 zipbreake Exp $
 */
#include "config.h"
...
static void check_pings(struct Event* ev) {
  ...
  for (i=0; i <= HighestFd; i++) {
	...
	send_reply(cptr, SND_EXPLICIT | ERR_BADPING,
		   ":Tu cliente de IRC puede que no sea compatible con este servidor de IRC.");
	send_reply(cptr, SND_EXPLICIT | ERR_BADPING,
		   ":Puedes encontrar clientes compatibles en %s",
