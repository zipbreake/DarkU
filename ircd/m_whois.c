 ...
 *
 * $Id: m_whois.c,v 1.20 2010/06/03 08:21:49 zipbreake Exp $
 */
...
static void do_whois(struct Client* sptr, struct Client *acptr, int parc)
{
...
#endif
  {
    ...
    int ocultacion_canales = ((IsBot(acptr) && IsInvisible(acptr)) || IsInvisiblePlus(acptr));
    int canales_ocultos = 0;
    len = 0;
    *buf = '\0';
    for (chan = user->channel; chan; chan = chan->next_channel)
    {
       ...
       if (ocultacion_canales && !(find_channel_member(sptr, chptr) || es_representante(sptr))) {
	 ...
	 *(buf + len) = '\0';
     ...
     if (canales_ocultos)
     {
         char temp[32];
         int longitud;
	 ircd_snprintf(0, temp, sizeof(temp), "...(%d)...", canales_ocultos);
  ...
  if (IsMe(a2cptr))
  {
    ...
    else if ((acptr != sptr) && feature_bool(FEAT_HIS_WHOIS_SERVERNAME) && !haspriv)
  ...
  if (user)
  {
	...
	/* Status oficiales, mostramos primero el mas importante. Siempre que deban mostrarse */
	if (IsBot(acptr))
		send_reply(sptr, RPL_WHOISBOT, name, "OFICIAL", feature_str(FEAT_NETWORK));
	...
	else if (IsLocAdmin(acptr))
		send_reply(sptr, RPL_WHOISRHPRIVS, name, "Admin de servidor", feature_str(FEAT_NETWORK));
	else if (IsHelpOp(acptr))
		send_reply(sptr, RPL_WHOISRHPRIVS, name, "Operador", feature_str(FEAT_NETWORK));
	else if (IsModera(acptr))
		send_reply(sptr, RPL_WHOISRHPRIVS, name, "Moderador", feature_str(FEAT_NETWORK));
	/* Es un bot de usuario */
	if (IsUserbot(acptr))
		send_reply(sptr, RPL_WHOISBOT, name, "de un Usuario", feature_str(FEAT_NETWORK));
	/* Tiene un status de whois especial */
	if (TieneRawStatus(acptr))
		send_reply(sptr, RPL_WHOISSTATUS, name, user->rawline);
	...
	if ((acptr == sptr) || IsAnOper(sptr) || es_representante(sptr) || IsModera(sptr))
	{
     ...
     */
    if (!IsInvisiblePlus(acptr) || (IsInvisiblePlus(acptr) && es_representante(sptr)) || (sptr == acptr)) { 
       if (MyConnect(acptr) && (!feature_bool(FEAT_HIS_WHOIS_IDLETIME) ||
			     sptr == acptr || IsAnOper(sptr) || parc >= 3))
      		send_reply(sptr, RPL_WHOISIDLE, name, CurrentTime - user->last, cli_firsttime(acptr));
    }
  }
}
