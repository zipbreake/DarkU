 ...
 *
 * $Id: m_burst.c,v 1.7 2009/10/03 08:16:23 zipbreake Exp $
 */
...
CMD_FUNC(ms_burst)
{
  ...
  if (!chptr->creationtime || chptr->creationtime > timestamp) {
    ...
    chptr->rhmode.mode &= ~(MODE_ADD | MODE_DEL | RHMODE_BADWORDS | RHMODE_NOCOLOR |
              RHMODE_RGSTRONLY | RHMODE_SECURE | RHMODE_SUSPEND | RHMODE_REGMOD |
              RHMODE_ONLYIRCOP | RHMODE_CHARLAS | RHMODE_NOCTCP);
