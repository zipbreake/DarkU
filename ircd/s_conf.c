 ...
 *
 * $Id: s_conf.c,v 1.2 2010/04/29 11:22:59 zipbreake Exp $
 */
#include "config.h"
...
static struct ExceptConf* exceptConfList;
...
check_limit_and_attach(struct Client* cptr, struct ConfItem* aconf)
{
  int number = 255;
  struct db_reg *reg;
  int clones_propios = 0;
  char su_ip[100];
  unsigned int su_ipn = ntohl(cli_ip(cptr).s_addr);

  sprintf(su_ip, "%u.%u.%u.%u",
	  (su_ipn >> 24) & 0xff,
	  (su_ipn >> 16) & 0xff,
	  (su_ipn >> 8) & 0xff,
	  su_ipn & 0xff);

  if ((reg = db_buscar_registro(BDD_ILINES, su_ip)) && (reg->valor) &&
	  (IsDigit(*reg->valor))) {
	int clones = atoi(reg->valor);
	if (clones) {
		clones_propios = 1;
		number = clones;
	}
  ...
  if (aconf->passwd && !clones_propios) {
  ...
  if (!clones_propios && (reg = db_buscar_registro(BDD_ILINES, ".")) && (reg->valor) &&
	  (IsDigit(*reg->valor))) {
	  int clones = atoi(reg->valor);
	  if (clones) {
		if (!aconf->passwd)
...
void conf_add_listener(const char* const* fields, int count)
{
  ...
  /*           port             vhost      mask  */
  add_listener(atoi(fields[4]), fields[2], fields[1], is_server, is_webchat, is_hidden);
}
...
/*
	E-Lines
	RyDeN - 20 Septiembre 2003

    Practicamente una copia de las K-Lines pero con utilidad inversa :)
*/
void conf_add_except(const char* const* fields, int count, int ip_except)
{
  struct ExceptConf* conf;

  if (count < 5 || EmptyString(fields[1]) || EmptyString(fields[3]))
    return;

  conf = (struct ExceptConf*) MyMalloc(sizeof(struct ExceptConf));
  assert(0 != conf);
  memset(conf, 0, sizeof(struct ExceptConf));
  ...
  if (!EmptyString(fields[2]))
  {
	  DupString(conf->opt_password, fields[2]);
  }
  ...
  if (ip_except) {
     ...
     */
    conf->address = inet_addr(ipname);
    Debug((DEBUG_DEBUG, "IPexcept: %s = %08x/%i (%08x)", ipname,
           conf->address, conf->bits, NETMASK(conf->bits)));
    conf->flags |= EXCEPT_FLAGS_IP;
  }

  conf->next = exceptConfList;
  exceptConfList = conf;
}
...
void conf_erase_except_list(void)
{
  ...
  for ( ; p; p = next) {
    ...
    MyFree(p->usermask);
	if (p->opt_password)
		MyFree(p->opt_password);
    MyFree(p);
  }
  exceptConfList = 0;
}

int find_except(struct Client *cptr)
{
  const char*        host;
  const char*        name;
  const char*        realname;
  ...
  for (except = exceptConfList; except; except = except->next) {
	  if (0 != match(except->usermask, name))
		  continue;

	  if ((except->opt_port != 0) && (except->opt_port != cli_port(cptr)))
		  continue;

	  if (!EmptyString(except->opt_password) && strcmp(except->opt_password, cli_passwd(cptr)))
		  continue;
	  
	  if (EmptyString(except->hostmask))
		  return 1;


	  if (except->flags & EXCEPT_FLAGS_REALNAME) {
		if (0 == match(except->hostmask + 2, realname))
			return 1;
	  }
	  else if (except->flags & EXCEPT_FLAGS_IP) {
		Debug((DEBUG_DEBUG, "ip: %08x network: %08x/%i mask: %08x",
             cli_ip(cptr).s_addr, except->address, except->bits, NETMASK(except->bits)));
		if ((cli_ip(cptr).s_addr & NETMASK(except->bits)) == except->address)
			return 1;
	  }
	  else if (0 == match(except->hostmask, host))
			return 1;
  }
  return 0;
}
  ...
  while (fbgets(line, sizeof(line) - 1, file)) {
    ...
    *dest = '\0';
    ...
    case 'd':  /* CONF_CRULEAUTO */
      ...
      break;
	case 'E':
	  conf_add_except(field_vector, field_count, 0);
	  aconf->status = CONF_ILLEGAL;
	  break;
	case 'e':
	  conf_add_except(field_vector, field_count, 1);
	  aconf->status = CONF_ILLEGAL;
	  break;
    case 'F':                /* Feature line */
    case 'f':
