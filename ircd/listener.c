 ...
 *
 *  $Id: listener.c,v 1.2 2010/04/29 11:22:59 zipbreake Exp $
 */
#include "config.h"
		...
		char* param)
{
  ...
  for (listener = ListenerPollList; listener; listener = listener->next) {
    if (port && port != listener->port)
      continue;
    flags[0] = (listener->server) ? 'S' : (listener->webchat) ? 'W' : 'C';
    if (listener->hidden) {
      if (!show_hidden)
        continue;
      flags[1] = 'H';
...
void add_listener(int port, const char* vhost_ip, const char* mask,
                  int is_server, int is_webchat, int is_hidden) 
{
  struct Listener* listener;
