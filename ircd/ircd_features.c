 ...
 *
 * $Id: ircd_features.c,v 1.10 2010/03/08 19:23:15 zipbreake Exp $
 */
#include "config.h"
...
#define F_S(type, flags, v_str, notify)					      \
  ...
  F_S(HIDDEN_HOST, FEAT_CASE, "users.gentechat.org", 0),
  ...
  F_S(HIS_SERVERNAME, 0, "*.gentechat.org", 0),
  F_S(HIS_SERVERINFO, 0, "Servidor de GenteChat", 0),
  F_S(RH_PSEUDO_SERVERNAME, 0, "irc.gentechat.org", 0),
  F_S(HIS_URLSERVERS, 0, "http://www.gentechat.org/servers.php", 0),
  F_S(AUTOJOIN, FEAT_NULL, 0, 0),

  /* Misc. random stuff */
  F_S(NETWORK, 0, "GenteChat", 0),
  F_S(URL_CLIENTS, 0, "ftp://ftp.undernet.org/pub/irc/clients", 0),

  /* --RyDeN
	Features de mostrar/ocultar el nombre de los servidores en el /map y en el /links
	*/
  F_B(SHOWSERVON_MAP, 0, 0, 0),
  F_B(SHOWSERVON_LINKS, 0, 0, 0),
  F_B(BDD_SUPPORT, 0, 1, 0),
  F_I(AUTH_TIMEOUT, 0, 60, 0),
  F_B(HIDDEN_SERVER, 0, 0, 0),

  /* Features de ZipBreake */
  F_B(SECURE_CHANNELS, 0, 0, 0),
  F_B(WEBCHAT_CONFIG, 0, 0, 0),
  F_B(SECURE_MODE, 0, 0, 0),
  F_B(LINK_CHANNELS, 0, 0, 0),
  F_S(HIDDEN_QUIT, FEAT_NULL, 0, 0),
  F_S(HIDDEN_QUIT_OPERS, FEAT_NULL, 0, 0),
