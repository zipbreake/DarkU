 ...
 *
 * $Id: m_kick.c,v 1.4 2009/02/08 14:47:17 zipbreake Exp $
 */
...
CMD_FUNC(m_kick)
{
  ...
  if (!is_chan_op(sptr, chptr) && !is_chan_owner(sptr, chptr)) {
	  struct Membership *cmpm;
	  if (!is_chan_halfop(sptr, chptr))
		return send_reply(sptr, ERR_CHANOPRIVSNEEDED, name);
	  if (!(cmpm = find_member_link(chptr, who))) return 0; /* ?????? No deberia ocurrir nunca... */
	  if (cmpm->status & (CHFL_CHANOP | CHFL_HALFOP))
		  return send_reply(sptr, ERR_CHANOPRIVSNEEDED, name);
  }

  if (is_chan_owner(who, chptr) && !IsServer(cptr) && !es_representante(sptr)) {
	  return send_reply(sptr, ERR_CHANOPRIVSNEEDED, name);
  }
...
CMD_FUNC(ms_kick)
{
   ...
   */
  if (!IsServer(sptr) && member && cli_from(who) != cptr && !IsBot(sptr) && 
      (!(sptr_link = find_member_link(chptr, sptr)) ||
	  (!IsChanOp(sptr_link) && !IsChanHalfop(sptr_link))))
  {
