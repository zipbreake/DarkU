 ...
 *
 * $Id: whocmds.c,v 1.11 2010/06/16 10:14:50 zipbreake Exp $
 */
#include "config.h"
            ...
            int fields, char* qrt)
{
  ...
  if (!fields || (fields & WHO_FIELD_SER))
  {
    ...
    if (IsMe(cli_user(acptr)->server))
    {
      ...
      if (feature_bool(FEAT_HIS_WHO_SERVERNAME) && !haspriv)
  ...
  if (!fields || (fields & WHO_FIELD_FLA))
  {
    *(p1++) = ' ';
    if (cli_user(acptr)->away)
      ...
      *(p1++) = 'H';
    if (((IsAnOper(acptr) || es_representante(acptr) || IsModera(acptr)))/* &&
