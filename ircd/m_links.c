 ...
 *
 * $Id: m_links.c,v 1.4 2009/11/19 22:15:36 zipbreake Exp $
 */
...
CMD_FUNC(m_links)
{
  ...
  if (feature_bool(FEAT_HIS_LINKS) && !(IsAnOper(sptr) || es_representante(sptr))) {
  ...
  if (parc > 2) {
	...
	if (!IsAnOper(sptr) && !es_representante(sptr) && !feature_bool(FEAT_SHOWSERVON_LINKS))
		showserv = 0;
  for (acptr = GlobalClientList, collapse(mask); acptr; acptr = cli_next(acptr))
  {
    ...
    send_reply(sptr, RPL_LINKS,
		(!showserv && IsHiddenserv(acptr)) ?
		feature_str(FEAT_HIS_SERVERNAME) : cli_name(acptr),
