 ...
 *
 * $Id: m_silence.c,v 1.1.1.1 2006/05/27 13:54:06 zipbreake Exp $
 */
...
CMD_FUNC(m_silence)
{
  ...
  if (parc < 2 || EmptyString(parv[1]) || (acptr = FindUser(parv[1]))) {
    if (!(cli_user(acptr)))
      return 0;
    if ((acptr != sptr) && !es_representante(sptr))
        return send_reply(sptr, ERR_SILECANTBESHOWN);
