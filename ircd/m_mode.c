 ...
 *
 * $Id: m_mode.c,v 1.8 2010/02/24 12:24:05 zipbreake Exp $
 */
...
m_mode(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (!(member = find_member_link(chptr, sptr)) ||
	  !(member->status & (CHFL_CHANOP | CHFL_HALFOP))) {
    if (IsLocalChannel(chptr->chname) && HasPriv(sptr, PRIV_MODE_LCHAN)) {
    ...
    } else if ((es_representante(sptr) || IsOper(sptr)) && strchr(parv[2], 'x')) {
      modebuf_init(&mbuf, sptr, cptr, chptr,
		   (MODEBUF_DEST_CHANNEL | /* Send mode to channel */
		    MODEBUF_DEST_SERVER  | /* Send mode to servers */
		    MODEBUF_DEST_XMODE  | /* Send Xmode */
		    MODEBUF_DEST_HACK4));  /* Send HACK(4) notice */
      mode_parse(&mbuf, cptr, sptr, chptr, parc - 2, parv + 2,
		 (MODE_PARSE_SET |    /* Set the mode */
      ...
      return modebuf_flush(&mbuf);
		 ...
		 /*if (es_representante(sptr) || IsOper(sptr) || IsModera(sptr)) {
			 modebuf_init(&mbuf, sptr, cptr, chptr,
				 (MODEBUF_DEST_CHANNEL | MODEBUF_DEST_SERVER));
			 mode_parse(&mbuf, cptr, sptr, chptr, parc - 2, parv + 2,
				 (MODE_PARSE_SET | MODE_PARSE_FORCE));
			 return modebuf_flush(&mbuf);
		 } else {*/
			modebuf_init(&mbuf, sptr, cptr, chptr,
				(MODEBUF_DEST_CHANNEL | MODEBUF_DEST_SERVER));
  ...
  flags = MODE_PARSE_SET;
  if (IsChanHalfop(member) && !IsChanOp(member))
	  flags |= MODE_PARSE_HALFOP | MODE_PARSE_NOTOPER;
  if (cli_flags(sptr) & FLAGS_CHSERV) {
	  flags |= MODE_PARSE_FORCE;
	  flags &= ~(MODE_PARSE_HALFOP | MODE_PARSE_NOTOPER);
  }
  ...
  modebuf_init(&mbuf, sptr, cptr, chptr,
	       (MODEBUF_DEST_CHANNEL | /* Send mode to channel */
		MODEBUF_DEST_SERVER)); /* Send mode to servers */
  mode_parse(&mbuf, cptr, sptr, chptr, parc - 2, parv + 2, flags);
  return modebuf_flush(&mbuf);
}
...
ms_mode(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (IsServer(sptr)) {
    ...
    if ((es_representante(sptr) || IsOper(sptr)) && strchr(parv[2], 'x')) {
      modebuf_init(&mbuf, sptr, cptr, chptr,
		   (MODEBUF_DEST_CHANNEL | /* Send mode to channel */
		    MODEBUF_DEST_SERVER  | /* Send mode to servers */
		    MODEBUF_DEST_XMODE  | /* Send Xmode */
      ...
      return modebuf_flush(&mbuf);
		...
		!(member->status & (CHFL_CHANOP | CHFL_HALFOP))) &&
		(!es_representante(sptr) && !IsModera(sptr) && !IsOper(sptr) && !IsBot(sptr))) {
      modebuf_init(&mbuf, sptr, cptr, chptr,
      ...
      mode_parse(&mbuf, cptr, sptr, chptr, parc - 2, parv + 2,
		...
		flags = MODE_PARSE_SET | MODE_PARSE_STRICT | MODE_PARSE_FORCE;
		if (!IsBot(sptr) && !es_representante(sptr) && !IsModera(sptr) && 
			member && IsChanHalfop(member) && !IsChanOp(member))
		{
			flags |= MODE_PARSE_HALFOP;
		}
