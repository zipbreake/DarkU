 ...
 * querycmds.h
 *
 * $Id: querycmds.h,v 1.1.1.1 2006/05/27 13:55:06 zipbreake Exp $
 */
...
#define Count_unknownbecomesclient(cptr, UserStats) \
  do { \
	...
	if (max_client_count >= max_global_count) { \
		max_global_count = max_client_count; \
		max_global_count_TS = TStime(); \
	} \
    ...
    { \
      ...
      if (max_connection_count % 10 == 0) \
        sendto_opmask_butone(0, SNO_OLDSNO, "Conexiones máximas: %d (%d clientes)", \
