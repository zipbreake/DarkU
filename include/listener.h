 ...
 *
 *
 * $Id: listener.h,v 1.2 2010/04/29 11:22:59 zipbreake Exp $
 */
...
struct Listener {
  ...
  unsigned char    webchat;            /* 1 if port is a webchat listener */
...
extern void        add_listener(int port, const char* vaddr_ip, 
                                const char* mask, int is_server, int is_webchat, 
                                int is_hidden);
