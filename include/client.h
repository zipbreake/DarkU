 ...
 *
 *
 * $Id: client.h,v 1.12 2010/04/29 11:22:59 zipbreake Exp $
 */
...
struct Client {
  ...
  unsigned int	 cli_rhflags;	/* Modos de usuario implementados para RedHispana */
  ...
  unsigned long   cli_negociacion;		/* Negociación de enlaces */

  /* Compresión de enlaces, extraído del ircu del IRC-Hispano http://www.irc-hispano.org */
  z_stream *comp_in;
  ...
  struct rc4_state *estado_rc4_in;
  struct rc4_state *estado_rc4_out;
};
...
#define cli_rhflags(cli)	((cli)->cli_rhflags)
...
#define cli_negociacion(cli) ((cli)->cli_negociacion)
...
#define STAT_UNKNOWN_WEBCHAT    0x100 /* Connect to webchat port */
...
#define IsUnknown(x)            (cli_status(x) & \
        (STAT_UNKNOWN | STAT_UNKNOWN_USER | STAT_UNKNOWN_SERVER | STAT_UNKNOWN_WEBCHAT))

#define IsWebchatPort(x)        (cli_status(x) == STAT_UNKNOWN_WEBCHAT )
...
#define IsClient(x)             (cli_status(x) & \
        (STAT_HANDSHAKE | STAT_ME | STAT_UNKNOWN |\
         STAT_UNKNOWN_USER | STAT_UNKNOWN_SERVER | STAT_UNKNOWN_WEBCHAT | STAT_SERVER | STAT_USER))
...
#define FLAGS_HIDDENSERV 0x10000 /* Servidor oculto en el /map */
...
/* Flags de RedHispana */
#define RHFLAGS_VIEWIP		0x01
#define RHFLAGS_HELPOP		0x02
#define RHFLAGS_WEBCHAT		0x04
#define RHFLAGS_COADM		0x08
#define RHFLAGS_ADM		0x10
#define RHFLAGS_MODERA		0x20
#define RHFLAGS_ONLYREG		0x40
#define RHFLAGS_INVISIBLE	0x80
#define RHFLAGS_REGNICK		0x100
#define RHFLAGS_USERBOT		0x200
#define RHFLAGS_BOT		0x400
#define RHFLAGS_SUSPENDED 	0x800
#define RHFLAGS_SECURE	 	0x1000
#define RHFLAGS_LOCADMIN	0x2000

#define RHFLAGS_RENAMED		0x100000
#define RHFLAGS_IPERSONALIZADA  0x400000
#define RHFLAGS_RAWSTATUS       0x800000

#define RHFLAGS (RHFLAGS_VIEWIP|RHFLAGS_INVISIBLE|RHFLAGS_LOCADMIN|RHFLAGS_HELPOP|RHFLAGS_COADM|RHFLAGS_ADM|RHFLAGS_MODERA|RHFLAGS_ONLYREG|RHFLAGS_REGNICK|RHFLAGS_USERBOT|RHFLAGS_BOT|RHFLAGS_SUSPENDED|RHFLAGS_SECURE|RHFLAGS_WEBCHAT)
#define SEND_UMODES (FLAGS_INVISIBLE|FLAGS_OPER|FLAGS_WALLOP|FLAGS_DEAF|FLAGS_CHSERV|FLAGS_DEBUG|FLAGS_ACCOUNT|FLAGS_HIDDENHOST)
#define ALL_UMODES (SEND_UMODES|FLAGS_SERVNOTICE|FLAGS_LOCOP)
#define FLAGS_ID (FLAGS_DOID|FLAGS_GOTID)

#define es_representante(x) (cli_rhflags(x) & (RHFLAGS_LOCADMIN | RHFLAGS_HELPOP | RHFLAGS_COADM | RHFLAGS_ADM | RHFLAGS_BOT))

/* Macros de los flags de RedHispana */

#define IsViewip(x)			(cli_rhflags(x) & RHFLAGS_VIEWIP)
#define IsInvisiblePlus(x)		(cli_rhflags(x) & RHFLAGS_INVISIBLE)
#define IsLocAdmin(x)			(cli_rhflags(x) & RHFLAGS_LOCADMIN)
#define IsHelpOp(x)			(cli_rhflags(x) & RHFLAGS_HELPOP)
#define IsCoadmin(x)			(cli_rhflags(x)	& RHFLAGS_COADM)
#define IsAdmin(x)			(cli_rhflags(x) & RHFLAGS_ADM)
#define IsModera(x)			(cli_rhflags(x) & RHFLAGS_MODERA)
#define IsOnlyreg(x)			(cli_rhflags(x) & RHFLAGS_ONLYREG)
#define IsRegnick(x)			(cli_rhflags(x) & RHFLAGS_REGNICK)
#define IsUserbot(x)			(cli_rhflags(x) & RHFLAGS_USERBOT)
#define IsBot(x)			(cli_rhflags(x) & RHFLAGS_BOT)
#define IsSuspended(x)			(cli_rhflags(x) & RHFLAGS_SUSPENDED)
#define IsSecured(x)			(cli_rhflags(x) & RHFLAGS_SECURE)
#define IsWebchatUser(x)		(cli_rhflags(x) & RHFLAGS_WEBCHAT)
#define TieneIpPersonalizada(x)         (cli_rhflags(x) & RHFLAGS_IPERSONALIZADA)
#define TieneRawStatus(x)	        (cli_rhflags(x) & RHFLAGS_RAWSTATUS)

#define SetViewip(x)			(cli_rhflags(x) |= RHFLAGS_VIEWIP)
#define SetInvisiblePlus(x)		(cli_rhflags(x) |= RHFLAGS_INVISIBLE)
#define SetLocAdmin(x)			(cli_rhflags(x) |= RHFLAGS_LOCADMIN)
#define SetHelpOp(x)			(cli_rhflags(x) |= RHFLAGS_HELPOP)
#define SetCoadmin(x)			(cli_rhflags(x) |= RHFLAGS_COADM)
#define SetAdmin(x)			(cli_rhflags(x) |= RHFLAGS_ADM)
#define SetModera(x)			(cli_rhflags(x) |= RHFLAGS_MODERA)
#define SetOnlyreg(x)			(cli_rhflags(x) |= RHFLAGS_ONLYREG)
#define SetRegnick(x)			(cli_rhflags(x) |= RHFLAGS_REGNICK)
#define SetUserbot(x)			(cli_rhflags(x) |= RHFLAGS_USERBOT)
#define SetBot(x)			(cli_rhflags(x) |= RHFLAGS_BOT)
#define SetSuspended(x)			(cli_rhflags(x) |= RHFLAGS_SUSPENDED)
#define SetSecured(x)			(cli_rhflags(x) |= RHFLAGS_SECURE)
#define SetWebchatUser(x)		(cli_rhflags(x) |= RHFLAGS_WEBCHAT)
#define SetIpPersonalizada(x)           (cli_rhflags(x) |= RHFLAGS_IPERSONALIZADA)
#define SetRawStatus(x)           	(cli_rhflags(x) |= RHFLAGS_RAWSTATUS)

#define ClearViewip(x)			(cli_rhflags(x) &= ~RHFLAGS_VIEWIP)
#define ClearInvisiblePlus(x)		(cli_rhflags(x) &= ~RHFLAGS_INVISIBLE)
#define ClearLocAdmin(x)		(cli_rhflags(x) &= ~RHFLAGS_LOCADMIN)
#define ClearHelpOp(x)			(cli_rhflags(x) &= ~RHFLAGS_HELPOP)
#define ClearCoadmin(x)			(cli_rhflags(x) &= ~RHFLAGS_COADM)
#define ClearAdmin(x)			(cli_rhflags(x) &= ~RHFLAGS_ADM)
#define ClearModera(x)			(cli_rhflags(x) &= ~RHFLAGS_MODERA)
#define ClearOnlyreg(x)			(cli_rhflags(x) &= ~RHFLAGS_ONLYREG)
#define ClearRegnick(x)			(cli_rhflags(x) &= ~RHFLAGS_REGNICK)
#define ClearUserbot(x)			(cli_rhflags(x) &= ~RHFLAGS_USERBOT)
#define ClearBot(x)			(cli_rhflags(x) &= ~RHFLAGS_BOT)
#define ClearSuspended(x)		(cli_rhflags(x) &= ~RHFLAGS_SUSPENDED)
#define ClearSecured(x)			(cli_rhflags(x) &= ~RHFLAGS_SECURE)
#define ClearWebchatUser(x)		(cli_rhflags(x) &= ~RHFLAGS_WEBCHAT)
#define ClearIpPersonalizada(x)         (cli_rhflags(x) &= ~RHFLAGS_IPERSONALIZADA)
#define ClearRawStatus(x)	        (cli_rhflags(x) &= ~RHFLAGS_RAWSTATUS)
...
#define IsHiddenserv(x)         (cli_flags(x) & FLAGS_HIDDENSERV)
...
#define SetHiddenserv(x)        (cli_flags(x) |= FLAGS_HIDDENSERV)
