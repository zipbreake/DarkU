 ...
 *
 * $Id: struct.h,v 1.2 2010/02/24 20:59:16 zipbreake Exp $
 */
...
struct Server {
  ...
  unsigned long   rhdbs_abiertas; /* Tablas de la db abiertas */
  unsigned long	  rhdbs2_abiertas;
  int bdd_version;
};
...
struct User {
  ...
  char               rawline[RAWLEN + 1];
  char               virtualhost[HOSTLEN + 1];
  char               realhost[HOSTLEN + 1];
  char               account[ACCOUNTLEN + 1];
  unsigned int       dbflags;         /* Los flags que posee dicho usuario en la RHDB */
};

#endif /* INCLUDED_struct_h */

char *get_virtualhost(struct Client *);
char *get_realhost(struct Client *);
